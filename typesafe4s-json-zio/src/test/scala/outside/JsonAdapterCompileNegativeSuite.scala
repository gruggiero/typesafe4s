...
class JsonAdapterCompileNegativeSuite extends munit.FunSuite {
  ...
  test("a raw zio-json ast.Json resolves no state encoder") {
    assert(
      compileErrors("""summon[typesafe4s.StateEncoder[zio.json.ast.Json]]""").nonEmpty,
      "a StateEncoder[zio.json.ast.Json] resolved — an unconditional given exists for a type that can hold numbers/booleans"
    )
  }
  ...
  test("the validated wrapper resolves a state encoder") {
    assert(compileErrors("""summon[typesafe4s.StateEncoder[typesafe4s.ziojson.JsonEntry]]""").isEmpty)
  }
}
