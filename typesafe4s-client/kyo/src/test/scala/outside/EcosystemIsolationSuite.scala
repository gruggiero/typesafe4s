...
// (the classpath-level half). This row carries the Kyo library; the other
...
class EcosystemIsolationSuite extends munit.FunSuite {
  ...
  test("the row's own ecosystem resolves and the other four do not") {
    assertEquals(
      compileErrors("""val x: kyo.<[Int, kyo.Abort[Throwable]] = ???"""),
      "",
      "this row's own ecosystem did not resolve"
    )
    assert(compileErrors("""val x: zio.ZIO[Any, Nothing, Int] = ???""").nonEmpty, "zio resolved on the kyo row")
    assert(compileErrors("""val x: cats.effect.IO[Int] = ???""").nonEmpty, "cats-effect resolved on the kyo row")
    assert(compileErrors("""val x: ox.Ox ?=> Int = ???""").nonEmpty, "ox resolved on the kyo row")
    assert(
      compileErrors("""val x: org.apache.pekko.stream.scaladsl.Source[Int, org.apache.pekko.NotUsed] = ???""").nonEmpty,
      "pekko resolved on the kyo row"
    )
  }
