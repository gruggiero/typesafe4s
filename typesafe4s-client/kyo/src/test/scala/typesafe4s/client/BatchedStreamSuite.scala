...
// ============================================================================
// TEST ORACLE — spec: question-batching — Kyo row: the published
// `systemOneBatched` returns `Stream[Evaluation[AnswerSet], Abort[Throwable]
// & Async & Scope]`; `Scope.run` collects the emitted evaluations, carrying
// every asked name. Carrier-level semantics are covered by the shared and
// parity suites; this suite covers the row's stream adapter: answers drain,
// a batch failure propagates through the stream, and an early `take` still
// reaches `run.close` — the Scope-registered release fires when the
// enclosing scope ends, even though Emit discards the producer
// continuation.
// ============================================================================
final class BatchedStreamSuite extends FunSuite {
  ...
  test("systemOneBatched-emits-every-answer-as-a-kyo-Stream") {
  ...
  // closes the session via the Scope-registered release: meter-blocked
  // workers see the abandon flag and no further exchange begins
  // (≤ 2 tolerates a worker that passed the flag check in the same
  // instant close landed — the shared suite's bound). The drain is
  // forked: a lazy stream only starts its session when it is consumed,
  // so the drain must already be running before the first exchange can
  // be observed parked on its gate.
