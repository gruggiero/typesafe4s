...
// ============================================================================
// TEST ORACLE — spec: question-batching — Cats Effect row: the published
// `systemOneBatched` returns an `fs2.Stream` and the emitted evaluations
// carry every asked name. Carrier-level semantics are covered by the shared
// and parity suites; this suite covers the row's stream adapter: answers
// drain, a batch failure propagates through the stream, and an early
// `take` still reaches `run.close`.
// ============================================================================
final class BatchedStreamSuite extends FunSuite {
  ...
  test("systemOneBatched-emits-every-answer-as-an-fs2-Stream") {
