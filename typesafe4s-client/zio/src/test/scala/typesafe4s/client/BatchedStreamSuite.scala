...
// ============================================================================
// TEST ORACLE — spec: question-batching — ZIO row: the published
// `systemOneBatched` returns a `ZStream` and the emitted evaluations carry
// every asked name. The carrier-level semantics are asserted per row by
// QuestionBatchingSuite/QuestionBatchingParitySuite; this suite covers the
// row's own stream adapter (unwrap + ensuring over the BatchRun session):
// answers drain, a batch failure propagates through the stream, and an
// early `take` still reaches `run.close`.
// ============================================================================
final class BatchedStreamSuite extends FunSuite {
  ...
  test("systemOneBatched-emits-every-answer-as-a-ZStream") {
