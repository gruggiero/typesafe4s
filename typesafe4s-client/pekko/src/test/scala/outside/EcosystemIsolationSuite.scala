...
// (the classpath-level half). This row carries the Pekko libraries (riding
// on scala.concurrent.Future); the other four ecosystems' types do not
// resolve at all — so nothing written against them could have slipped into
// the shared sources this row compiles.
//
...
class EcosystemIsolationSuite extends munit.FunSuite {
  ...
  test("the row's own ecosystem resolves and the other four do not") {
    assertEquals(
      compileErrors("""val x: org.apache.pekko.stream.scaladsl.Source[Int, org.apache.pekko.NotUsed] = ???"""),
      "",
      "this row's own ecosystem did not resolve"
    )
    assert(compileErrors("""val x: zio.ZIO[Any, Nothing, Int] = ???""").nonEmpty, "zio resolved on the pekko row")
    assert(compileErrors("""val x: cats.effect.IO[Int] = ???""").nonEmpty, "cats-effect resolved on the pekko row")
    assert(compileErrors("""val x: ox.Ox ?=> Int = ???""").nonEmpty, "ox resolved on the pekko row")
    assert(compileErrors("""val x: kyo.<[Int, kyo.Abort[Throwable]] = ???""").nonEmpty, "kyo resolved on the pekko row")
  }
