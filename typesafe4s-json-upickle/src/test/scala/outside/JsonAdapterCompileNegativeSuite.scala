...
class JsonAdapterCompileNegativeSuite extends munit.FunSuite {
  ...
  test("a raw ujson.Value resolves no state encoder") {
    assert(
      compileErrors("""summon[typesafe4s.StateEncoder[ujson.Value]]""").nonEmpty,
      "a StateEncoder[ujson.Value] resolved — an unconditional given exists for a type that can hold numbers/booleans"
    )
  }
  ...
  test("the validated wrapper resolves a state encoder") {
    assert(compileErrors("""summon[typesafe4s.StateEncoder[typesafe4s.upickle.JsonEntry]]""").isEmpty)
  }
}
