...
import typesafe4s.upickle.JsonEntry

// ============================================================================
// TYPED CONTRACT — spec: json-adapters (change: add-typesafe4s-sdk, spec 9/10)
//
// Gate 1 artifact. Module: typesafe4s-upickle.
//
...
// for ujson.Value could not honestly exist; the wrapper discharges the
...
private object JsonAdapterTypeContract {
  ...
  // the checked crossing — the only way a ujson.Value becomes a JsonEntry
  def witnessOf(value: ujson.Value): Either[String, JsonEntry] =
    JsonEntry.of(value)
...
//   `nothing-stays-nothing` — ujson.Null converts to Entry.nothing
//
// Compile-Negative Obligation — outside/JsonAdapterCompileNegativeSuite:
//   * `summon[StateEncoder[ujson.Value]]` must NOT resolve — no
